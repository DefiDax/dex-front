...
export function ListSelect({ onDismiss, onBack }: { onDismiss: () => void; onBack: () => void }) {
  ...
  const dispatch = useDispatch<AppDispatch>()
  const lists = useSelector<AppState, AppState['lists']['byUrl']>(state => state.lists.byUrl) // updater 中加载的吧？？
  ...
  const fetchList = useFetchListCallback() // 用于执行拉取的函数

  const handleAddList = useCallback(() => {
    if (adding) return
    setAddError(null)
    fetchList(listUrlInput) // 根据输入查询
      .then(() => {
        setListUrlInput('')
        ReactGA.event({
          category: 'Lists',
