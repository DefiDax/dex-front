...
describe('utils', () => {
  describe('#gethpbscanLink', () => {
    it('correct for tx', () => {
      expect(gethpbscanLink(1, 'abc', 'transaction')).toEqual('https://hpbscan.io/tx/abc')
    })
    it('correct for token', () => {
      expect(gethpbscanLink(1, 'abc', 'token')).toEqual('https://hpbscan.io/token/abc')
    })
    it('correct for address', () => {
      expect(gethpbscanLink(1, 'abc', 'address')).toEqual('https://hpbscan.io/address/abc')
    })
    it('unrecognized chain id defaults to mainnet', () => {
      expect(gethpbscanLink(2, 'abc', 'address')).toEqual('https://hpbscan.io/address/abc')
    })
    it('ropsten', () => {
      expect(gethpbscanLink(3, 'abc', 'address')).toEqual('https://ropsten.hpbscan.io/address/abc')
    })
    it('enum', () => {
      expect(gethpbscanLink(ChainId.RINKEBY, 'abc', 'address')).toEqual('https://rinkeby.hpbscan.io/address/abc')
    })
  })
